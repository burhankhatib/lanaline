...
export default function Footer() {
    ...
    return (
        <footer className="container flex flex-col items-center justify-center w-full h-16 max-w-6xl p-4 mx-auto bg-primary ">
            ...
            <div className="flex items-center justify-center w-full gap-2 text-white">
                ...
                <div className="flex items-center gap-2">
                    {socialLinks.map((link) => (
                        <Link href={link.url} key={link.name} target="_blank" className="hover:text-secondary">
                            {link.icon}
                        </Link>
                    ))}
                </div>
